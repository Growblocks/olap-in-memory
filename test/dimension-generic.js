...
describe('GenericDimension', () => {
  ...
  it('should compute items for all attributes', () => {
    assert.deepEqual(dimension.getItems(), [
      'paris',
      'toulouse',
      'madrid',
      'beirut',
    ]);
    assert.deepEqual(dimension.getItems('city'), [
      'paris',
      'toulouse',
      'madrid',
      'beirut',
  ...
  it('should compute child items for all attributes', () => {
    ...
    assert.equal(
      dimension.getGroupItemFromRootItem('cityNumLetters', 'madrid'),
      '6',
    );
    assert.equal(
      dimension.getGroupItemFromRootItem('country', 'madrid'),
      'spain',
    );
    assert.equal(
      dimension.getGroupItemFromRootItem('continent', 'madrid'),
      'europe',
    );
  });

  it('should compute child indexes', () => {
  ...
  it('should raise when intersecting dimensions with no common attribute', () => {
    const otherDimension = new GenericDimension('location', 'postalcode', [
      '75018',
      '75019',
    ]);
  ...
  it('should union', () => {
    ...
    assert.deepEqual(
      result.getGroupItemFromRootItem('country', 'lyon'),
      'france',
    );
    assert.deepEqual(
      result.getGroupItemFromRootItem('country', 'paris'),
      'france',
    );
